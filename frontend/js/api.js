...
class API {
  constructor() {
    // Use current host/origin — works on localhost:5000 and production (Vercel) alike.
    // The Flask backend serves both the API (/api/*) and the frontend from the same origin,
  ...
  /**
   * Get CSRF token from cookie or cached value
  ...
  async forgotPassword(email) {
    return this.request(`${this.authURL}/forgot-password`, {
      method: 'POST',
  ...
  async resetPassword(token, password) {
    return this.request(`${this.authURL}/reset-password`, {
      method: 'POST',
   ...
   * @returns {Promise} Price range response
   */
  async getPriceRange() {
   ...
   * @param {Object} orderData - Order data including guest info
   * @returns {Promise} Order response
   */
  async createGuestOrder(orderData) {
    return this.post('/orders/guest', orderData);
  }
   ...
   * @param {number} productId - Product ID
   * @param {number} limit - Number of products to fetch
   * @returns {Promise} Related products response
   */
  ...
  async moveWishlistToCart(productId) {
    return this.post(`/wishlist/move-to-cart/${productId}`);
  }
  ...
  async validatePromoCode(code, orderAmount) {
    return this.post('/promo/validate', { code, order_amount: orderAmount });
  }
   ...
   * @param {string} code - Promo code
   * @param {number} orderAmount - Order amount
   * @returns {Promise} Apply response
   */
  async applyPromoCode(code, orderAmount) {
    return this.post('/promo/apply', { code, order_amount: orderAmount });
  }
  ...
  async removePromoCode() {
    return this.post('/promo/remove');
  }

  /**
   * Get currently applied promo code
   * @returns {Promise} Current promo response
   */
  async getCurrentPromo() {
    return this.get('/promo/current');
  }

  // Payment Methods
  /**
   * Get payment configuration
   * @returns {Promise} Payment config response
   */
  async getPaymentConfig() {
    return this.get('/payment/config');
  }
  ...
  async initializePayment(paymentData) {
    return this.post('/payment/initialize', paymentData);
  }
  ...
  async verifyPayment(reference) {
    return this.get(`/payment/verify/${reference}`);
  }
   ...
   * @returns {Promise} Delivery zones response
   */
   ...
   * @returns {Promise} Delivery check response
   */
  async checkDeliveryArea(area) {
    return this.post('/delivery/check', { area });
  }

  /**
   * Calculate delivery fee for an order
   * @param {number} zoneId - Delivery zone ID
   * @param {number} subtotal - Order subtotal
   * @returns {Promise} Delivery fee calculation response
   */
  async calculateDeliveryFee(zoneId, subtotal) {
    return this.post('/delivery/calculate', { zone_id: zoneId, subtotal });
  }
}
